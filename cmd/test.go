...
Flags:
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err := runDNSQueryAndCompare(testType, expectedValues, dnsServer, host); err != nil {
			ui.PrintMsgWithStatus("FAIL", "red", "Test failed, check above\n")
			os.Exit(1)
		} else {
...
func parseFlags(cmd *cobra.Command) (string, []string, string, string, error) {
	...
	if testType == "" || len(expectedValues) == 0 || dnsServer == "" || host == "" {
		return "", nil, "", "", fmt.Errorf("provide a test type, host, expected values, and DNS server")
	}
...
func init() {
	...
	testCmd.Flags().StringP("server", "s", "", "DNS server to query (1.1.1.1)")
	testCmd.Flags().StringP("host", "H", "", "The host you want to look up (example.com)")
}
