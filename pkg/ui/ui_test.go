...
func TestPrintDashes(t *testing.T) {
	output := captureOutput(PrintDashes, false)
...
func TestPrintErrMsgWithStatus(t *testing.T) {
	type args struct {
		status string
		color  string
		format string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			output := captureOutput(func() {
				PrintErrMsgWithStatus(tt.args.status, tt.args.color, tt.args.format, tt.args.a...)
			}, true)
			if output != tt.want {
				t.Errorf("PrintErrMsgWithStatus() = %v, want %v", output, tt.want)
			}
		})
	}
}

func captureOutput(f func(), isError bool) string {
	r, w, _ := os.Pipe()
